...
from utils.common import event_map
...
@router.api_route(
    "/feedback/metadata", methods=["GET"], tags=["Feedback"]
)
def get_feedback_metadata(response:Response, plant_id:str='gml-luh-001'):
    results = {}
    try:
        plant_info = PlantInfo.objects.get(plant_id=plant_id)
        waste_feedback = WasteFeedback()
        fields = waste_feedback._meta.get_fields()
        
        ignore_fields = ['id', 'created_at', 'updated_at']
        fields = [f for f in fields if f.name not in ignore_fields]
        colums = {}
        for f in fields:
            colums[f.name] = {
                'title': f.name,
...
@router.api_route(
    "/feedback/{event_uid}", methods=["POST"], tags=["Feedback"], description=insert_feedback_description,
)
def insert_feedback(response: Response, event_uid:str, request:dict):
    results = {}
    try:
        ...
        if WasteFeedback.objects.filter(event_uid=event_uid, user_id=user_id).exists():
            ...
            if 'comment' in request.keys():
                waste_feedback.comment = request.get('comment')
                
            if 'rating' in request.keys():
                waste_feedback.rating = request.get('rating')
            
            if 'meta_info' in request.keys():
                waste_feedback.meta_info = request.get('meta_info')
            ...
            if 'ack_status' in request.keys():
                waste_feedback.ack_status = request.get('ack_status')           
