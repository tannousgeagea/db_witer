...
from database.models import PlantInfo, EdgeBoxInfo, WasteHotSpot
...
@router.api_route(
    "/hotspot/metadata", methods=["GET"], tags=["Hotspot"]
)
def get_hotspot_metadata(response: Response, plant_id:str="gml-luh-001"):
    results = {}
    try:
...
@router.api_route(
    "/hotspot/data", methods=["GET"], tags=["Hotspot"]
)
def get_hotspot_data(response: Response, plant_id:str='gml-luh-001',from_date:datetime=None, to_date:datetime=None):
    results = {}
    items_per_page = 15
    try:
        plant_info = PlantInfo.objects.get(plant_id=plant_id)
        ...
        waste_hotspot = WasteHotSpot.objects.filter(created_at__range=(from_date, to_date )).order_by('-created_at')
        rows = []
        total_record = len(waste_hotspot)
...
@router.api_route(
    "/hotspot/delivery/{delivery_id}", methods=["GET"], tags=["Hotspot"]
)
def get_hotspot_data_by_delivery(response: Response, delivery_id:str):
    results = {}
    
    try:
        waste_hotspot = WasteHotSpot.objects.filter(delivery_id=delivery_id).order_by('-created_at')
        rows = []
        for wi in waste_hotspot:
            description = wi.meta_info.get('description')
            row = {
